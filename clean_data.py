...
need_day = np.array(SP500_stock["CME"]["stock_price"]['Date'])
...
features = {}
for target in SP500_stock.keys():
    features[target] = SP500_stock[target]["stock_price"].iloc[30:,6:7].reset_index(drop=True)
...
def before_day(week):
    ...
    for w in range(week):
        train_x = []
        for tr_ind in range(len(train_data["CME"])-7-(week-2)-1):
            tr = []
            for target in SP500_stock.keys():
    ...
    for w in range(week):
        test_x = []
        for te_ind in range(len(test_data["CME"])-7-(week-2)-1):
            te = []
            for target in SP500_stock.keys():
